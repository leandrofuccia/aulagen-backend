...
A aplicação possui 2 perfis de uso (Professor e Aluno), com autenticação. As funcionalidades comuns a ambos os perfis são compartilhadas (visualização de postagens existentes de todos os professores autores) e as demais permanecem restritas ao ocupacao Professor (criar, editar e excluir postagens).
...
**Tabela ocupacao**: armazena os diferentes tipos de perfis de usuários (ex.: professor, aluno).
...
CREATE TABLE ocupacao (
    id int4 PRIMARY KEY,
    ocupacao varchar NOT NULL UNIQUE
);
```
...
Para utilizar os demais endpoints, é necessário criar um Usuário e definir seu ocupacao.
